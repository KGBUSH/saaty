    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            original_latency = int(
                request.args.get('originalLatency', 0))  # 单位：s，不必需
            ...
            # 运营在雨天会有额外的延时配置
            heavy_weather_latency = int(
                request.args.get('heavyWeatherTime', 0)) * 60  # 单位：s，不必需
        ...
        dynamic_time_enable = True

        # 检查黑名单
        if isinstance(label_ids, str):
            label_list = map(lambda x: int(x), label_ids.strip().split(','))
            label_black_list = app.config.get("ORDER_BLACK_LIST_LABEL", [])
            blacklist_label_id = set(label_list).intersection(
                set(label_black_list))
            if blacklist_label_id:
                dynamic_time_enable = False

        if dynamic_time_enable:
            # 优先检查人工收集 POI 延时
        ...
        algoKafkaLogger.info(kafka_event.DYNAMIC_POI_LATENCY_ONLINE_CHECK,
                             request_respons_info)
